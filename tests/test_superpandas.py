...
def random_integers(): return [random.randint(1, 100) for _ in range(3)]
...
def test_add_foreign_key():
    ...
    sdf2 = SuperDataFrame(sample_df(), name='Table2')
    pdp = PandaPack(sdf=[sdf1, sdf2])
    pdp.add_foreign_key(src_sdf='Table1', src_column='A',
                        tgt_sdf='Table2', tgt_column='B')
    assert len(pdp.foreign_keys) == 1
    fk = ForeignKey(src_sdf='Table1', src_column='B',
                    tgt_sdf='Table2', tgt_column='A')
...
def test_from_json():
    ...
    pdp.add_foreign_key(src_sdf='Table1', src_column='A',
                        tgt_sdf='Table2', tgt_column='B')
    arch = pdp.to_json()
    new_pdp = PandaPack.from_json(arch)
...
def test_update_sdf():
    ...
    df2 = sample_df()
    sdf1 = SuperDataFrame(df1, name='Table1')
    pdp = PandaPack(sdf=sdf1)
    pdp.update_sdf('Table1', df2)
    assert pdp.get_sdf('Table1').equals(SuperDataFrame(df2, name='Table1'))
